...
impl ContentParser for YamlParser {
    ...
    fn extensions(&self) -> &[&str] { &["yaml", "yml"] }
    ...
    fn parse(&self, file_path: &str, source: &str, repo: &str) -> Result<(Vec<CodeEntity>, Vec<CodeRelation>)> {
        ...
        extract_yaml_value(&value, file_path, repo, &file_qname, "", &mut entities, &mut relations, 0);
        Ok((entities, relations))
    }
}

fn extract_yaml_value(
    value: &serde_yaml::Value, file_path: &str, repo: &str,
    ...
    if let serde_yaml::Value::Mapping(map) = value {
        for (k, v) in map {
            ...
            entities.push(CodeEntity {
                ...
                signature: None, body: body_str, body_hash: None, language: "yaml".to_string(),
            });
