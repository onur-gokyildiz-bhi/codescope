...
use crate::helpers::{
    ...
    derive_scope_from_file_path, link_cross_session_topics,
    load_known_entities,
};
use crate::nlp;
use crate::params::*;
        ...
        match parsed.intent {
            ...
            nlp::Intent::Search(entity) => {
                ...
                for term in &parsed.search_terms {
                    ...
                    // Search functions by name, qualified_name, and signature
                    if search_functions {
                        let surql = "SELECT name, qualified_name, file_path, start_line, end_line, signature \
                                     ...
                                     LIMIT 25";
                        if let Ok(mut r) = ctx.db.query(surql).bind(("term", term_owned.clone())).await {
                            let rows: Vec<serde_json::Value> = r.take(0).unwrap_or_default();
                            all_results.extend(rows);
                        }
                    }
                    // Search classes/structs
                    if search_classes {
                ...
                // Score: count how many search terms match in name
                all_results.sort_by(|a, b| {
                    let score = |v: &serde_json::Value| -> usize {
                        ...
                        parsed.search_terms.iter().filter(|t| name.contains(t.as_str()) || qname.contains(t.as_str())).count()
                    };
                    score(b).cmp(&score(a))
                });
            ...
            nlp::Intent::CallGraph(direction) => {
                ...
                match direction {
                    nlp::CallDirection::Callers => {
                        match gq.find_callers(func_name).await {
                            Ok(results) => {
                                ...
                                if results.is_empty() { out.push_str("No callers found.\n"); }
                                out
                            }
                            Err(e) => format!("Error finding callers: {}", e),
                        }
                    }
                    nlp::CallDirection::Callees => {
                        match gq.find_callees(func_name).await {
                            Ok(results) => {
                                ...
                                if results.is_empty() { out.push_str("No callees found.\n"); }
                                out
                            }
            ...
            // ── In File ──────────────────────────────────────────
            nlp::Intent::InFile => {
                let path = parsed.file_path.as_deref()
                ...
                match gq.file_entities(path).await {
                    Ok(results) => {
                        ...
                        if results.is_empty() { out.push_str("No entities found. Check the file path.\n"); }
                        out
                    }
                    Err(e) => format!("Error: {}", e),
                }
