...
impl GraphRagServer {
    ...
    async fn api_changelog(&self, Parameters(_params): Parameters<ApiChangelogParams>) -> String {
        let ctx = match self.gated_ctx_named("api_changelog").await {
            Ok(c) => c,
            Err(e) => return e,
        };
    ...
    async fn community_detection(
        ...
        let ctx = match self.gated_ctx_named("community_detection").await {
            Ok(c) => c,
            Err(e) => return e,
        };
    ...
    async fn export_obsidian(
        ...
        let ctx = match self.gated_ctx_named("export_obsidian").await {
            Ok(c) => c,
            Err(e) => return e,
        };
    ...
    async fn capture_insight(
        ...
        let ctx = match self.gated_ctx_named("capture_insight").await {
            Ok(c) => c,
            Err(e) => return e,
        };
    ...
    async fn suggest_structure(
        ...
        let ctx = match self.gated_ctx_named("suggest_structure").await {
            Ok(c) => c,
            Err(e) => return e,
        };
