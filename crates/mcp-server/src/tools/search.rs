...
impl GraphRagServer {
    ...
    async fn find_callers(&self, Parameters(params): Parameters<FindCallersParams>) -> String {
        let ctx = match self.gated_ctx_named("find_callers").await {
            Ok(c) => c,
            Err(e) => return e,
        };
    ...
    async fn find_callees(&self, Parameters(params): Parameters<FindCalleesParams>) -> String {
        let ctx = match self.gated_ctx_named("find_callees").await {
            Ok(c) => c,
            Err(e) => return e,
        };
        let gq = GraphQuery::new(ctx.db);
    ...
    async fn graph_stats(&self) -> String {
        let ctx = match self.gated_ctx_named("graph_stats").await {
            Ok(c) => c,
            Err(e) => return e,
        };
        let gq = GraphQuery::new(ctx.db);
    ...
    async fn raw_query(&self, Parameters(params): Parameters<RawQueryParams>) -> String {
        let ctx = match self.gated_ctx_named("raw_query").await {
            Ok(c) => c,
            Err(e) => return e,
        };
        let gq = GraphQuery::new(ctx.db);
